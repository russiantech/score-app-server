...
#     redis: RedisService = Depends(get_redis_service),
# ) -> dict:
#     """Initiate password reset process with code"""
#     try:
#         client_ip = get_client_ip(request)
...
#         redis_key = f"password_reset:{user.email.lower()}"
#         if not redis.set(redis_key, reset_data, expiry=900):
#             raise HTTPException(
#                 status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
#                 detail="Service temporarily unavailable. Please try again."
#             )
...
#         _, allowed = redis.increment_rate_limit(
#             f"reset_password_attempts:{data.email}",
#             window=300,
#             limit=5
#         )
...
#         # Get reset data from Redis using email
#         redis_key = f"password_reset:{data.email.lower()}"
...
#         # Verify reset code
#         if reset_data.get('reset_code') != data.reset_code.upper():
#             raise HTTPException(
#                 status_code=status.HTTP_400_BAD_REQUEST,
#                 detail="Invalid reset code. Please check your email and try again."
#             )
