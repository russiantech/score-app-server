...
Let's switch to start_tls automatically if ssl failes, vice versa...?
And maybe also try sending via API before final fails
...
#     msg["Subject"] = subject
#     # msg["From"] = f"Dunistech Academy <no-reply@dunistech.ng>"
...
#     logger.info(f"Preparing SMTP email to: {msg['To']}, from: {msg['From']}, subject: {subject}, message: {text_body[:50]}...")
#     logger.info(f"params: host={app_config.mail_config.smtp_host}, port={app_config.mail_config.smtp_port}, password={app_config.mail_config.smtp_password}, user={app_config.mail_config.smtp_username}")
    
#     await aiosmtplib.send(
#         msg,
#         hostname=app_config.mail_config.smtp_host,
#         port=app_config.mail_config.smtp_port,
#         # username=app_config.mail_config.smtp_username,
#         username=app_config.mail_config.system_email_address,
#         password=app_config.mail_config.smtp_password,
#         start_tls=False,
#     )
...
async def _smtp_send_async_1(
    ...
    # Fix FROM address - MUST be the same as SMTP username
    msg["Subject"] = subject
    msg["From"] = f"Dunistech Academy <{app_config.mail_config.smtp_username}>"
    msg["To"] = ", ".join(recipients)
    ...
    try:
        await aiosmtplib.send(
            msg,
            hostname=app_config.mail_config.smtp_host,
            port=app_config.mail_config.smtp_port,  # Should be 587
            username=app_config.mail_config.smtp_username,
...
async def send_password_reset_email(
    ...
    try:
        # Generate reset link
        reset_link = f"{config.app_config.frontend_url}/auth/reset-password?token={token}"
