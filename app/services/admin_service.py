...
#     """Get comprehensive admin statistics - Optimized"""
...
#     # Count by roles
#     total_students = db.query(User).join(User.roles).filter(
#         User.roles.any(name='student')
#     ).count()
    
#     total_tutors = db.query(User).join(User.roles).filter(
#         User.roles.any(name='tutor')
#     ).count()
    
#     total_parents = db.query(User).join(User.roles).filter(
#         User.roles.any(name='parent')
#     ).count()
    
#     total_admins = db.query(User).join(User.roles).filter(
...
#     active_tutors = db.query(User).join(User.roles).filter(
#         User.roles.any(name='tutor'),
#         User.is_active == True
#     ).count()
...
#     avg_class_size = db.query(func.avg(avg_subquery.c.student_count)).scalar() or 0
    
#     # Students without parents - MY FIXED APPROACH
#     students_with_parents = db.query(distinct(parent_students.c.student_id)).count()
