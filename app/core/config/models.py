...
#     media_location: str = "/static/media"
...
#     api_url: str = "http://localhost:8001"  # Full API URL
...
class ContentDeliveryConfig(BaseModel):
    type: Literal["filesystem", "s3api", "cloudinary"] = "filesystem"
    filesystem_base_path: str = "./uploads"
    max_file_size_mb: int = 100
...
# ============================================================
# HOSTING
# Single source of truth for address/port/URL configuration.
#
# Set these three env vars:
#   APP_DOMAIN  — e.g. "localhost" or "api.dunistech.ng"
#   APP_PORT    — e.g. 8001  (ignored when ssl=True on a standard port)
#   APP_SSL     — true / false
#
# Everything else (api_url, cookie domain) is derived automatically.
# Never set APP_API_URL — it is computed, not configurable.
# ============================================================
...
class HostingConfig(BaseModel):
    ...
    # ----------------------------------------------------------
    # COMPUTED — derived from domain + port + ssl.
    # This is the single authoritative URL for the API.
    # Used by FileSystemStorage, email templates, OAuth callbacks, etc.
    # ----------------------------------------------------------
    @computed_field  # type: ignore[misc]
    @property
    def api_url(self) -> str:
        scheme = "https" if self.ssl else "http"
        standard_port = (443 if self.ssl else 80)
        if self.port == standard_port:
            return f"{scheme}://{self.domain}"
        return f"{scheme}://{self.domain}:{self.port}"

    # ----------------------------------------------------------
    # Keep cookie domain in sync with the API domain automatically
    # ----------------------------------------------------------
    @model_validator(mode="after")
    def sync_cookie_domain(self) -> "HostingConfig":
        if self.cookie_config.domain == "localhost" and self.domain != "localhost":
            self.cookie_config.domain = self.domain
        return self
...
class SecurityConfig(BaseModel):
    auth_jwt_secret_key: str  # required
    jwt_algorithm: str = "HS256"
    ...
    cors_allowed_origins: list[str] = ["http://localhost:3000", "http://localhost:8001"]
...
class ChromaDBConfig(BaseModel):
    ...
    collection_name: str = "academy_documents"
...
class MailConfig(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_provider_requirements(self) -> "MailConfig":
        checks: dict[str, list] = {
            "smtp":     [self.smtp_host, self.smtp_username, self.smtp_password],
            "resend":   [self.resend_api_key],
            "sendgrid": [self.sendgrid_api_key],
            "mailgun":  [self.mailgun_api_key, self.mailgun_domain],
            "console":  [],
        }
        required = checks.get(self.provider, [])
        if not all(required):
            raise ValueError(
                f"Incomplete credential configuration for mail provider '{self.provider}'"
            )
        return self
...
class AppConfig(BaseModel):
    ...
    @property
    def is_testing(self) -> bool:
        conn = self.database_config.sql_connection_string or ""
        return self.general_config.development_mode and "test" in conn.lower()
